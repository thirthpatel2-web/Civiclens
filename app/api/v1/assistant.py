...
@router.post("/ask")
def ask(body: AskBody, ctx: AuthContext = USE, c: AppContainer = Depends(get_container), _: None = Depends(limited("expensive"))) -> dict:  # type: ignore[type-arg]
    return c.assistant.ask(ctx, body.question, conversation_id=body.conversation_id, language=body.language)


@router.post("/chat")
def chat(body: AskBody, ctx: AuthContext = USE, c: AppContainer = Depends(get_container), _: None = Depends(limited("expensive"))) -> dict:  # type: ignore[type-arg]
    return c.assistant.ask(ctx, body.question, conversation_id=body.conversation_id, language=body.language)


@router.post("/conversations")
def conversations(ctx: AuthContext = USE, c: AppContainer = Depends(get_container)) -> dict:  # type: ignore[type-arg]
...
def messages(conversation_id: str, ctx: AuthContext = USE, c: AppContainer = Depends(get_container)) -> dict:  # type: ignore[type-arg]
