...
def _session_payload(c: AppContainer, token: str, ctx: AuthContext, uow_info: dict) -> dict:  # type: ignore[type-arg]
...
def _user_info(c: AppContainer, uow: object, user_id: str) -> dict:  # type: ignore[type-arg]
...
def register(body: RegisterBody, c: AppContainer = Depends(get_container)) -> dict:  # type: ignore[type-arg]
...
def _login(body: LoginBody, request: Request, c: AppContainer, *, privileged_only: bool, client: str = "web"):  # type: ignore[no-untyped-def]
...
@router.post("/login")
def login(body: LoginBody, request: Request, response: Response, c: AppContainer = Depends(get_container)) -> dict:  # type: ignore[type-arg]
...
def mobile_login(body: LoginBody, request: Request, c: AppContainer = Depends(get_container)) -> dict:  # type: ignore[type-arg]
...
def admin_login(body: LoginBody, request: Request, response: Response, c: AppContainer = Depends(get_container)) -> dict:  # type: ignore[type-arg]
...
@router.get("/me")
def me(request: Request, ctx: AuthContext = Depends(get_ctx), c: AppContainer = Depends(get_container)) -> dict:  # type: ignore[type-arg]
...
@router.post("/logout")
def logout(request: Request, response: Response, ctx: AuthContext = Depends(get_ctx), c: AppContainer = Depends(get_container)) -> dict:  # type: ignore[type-arg]
...
def forgot(body: ForgotBody, c: AppContainer = Depends(get_container)) -> dict:  # type: ignore[type-arg]
...
def reset(body: ResetBody, c: AppContainer = Depends(get_container)) -> dict:  # type: ignore[type-arg]
    run_in_uow(c, lambda uow: c.auth_for(uow).reset_password(body.token, body.new_password), commit_on_error=True)
    return {"ok": True}


@router.post("/password/change")
def change_password(body: ChangePasswordBody, ctx: AuthContext = Depends(get_ctx), c: AppContainer = Depends(get_container)) -> dict:  # type: ignore[type-arg]
...
def mfa_enroll(ctx: AuthContext = Depends(get_ctx), c: AppContainer = Depends(get_container)) -> dict:  # type: ignore[type-arg]
    def op(uow):  # type: ignore[no-untyped-def]
...
def mfa_confirm(body: OtpBody, request: Request, ctx: AuthContext = Depends(get_ctx), c: AppContainer = Depends(get_container)) -> dict:  # type: ignore[type-arg]
    token = session_token(request)[0]
...
def mfa_disable(body: DisableMfaBody, ctx: AuthContext = Depends(get_ctx), c: AppContainer = Depends(get_container)) -> dict:  # type: ignore[type-arg]
    def op(uow):  # type: ignore[no-untyped-def]
...
def bootstrap(body: BootstrapBody, c: AppContainer = Depends(get_container)) -> dict:  # type: ignore[type-arg]
