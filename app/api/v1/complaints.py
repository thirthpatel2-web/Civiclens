...
def create(body: ComplaintBody, ctx: AuthContext = Depends(guard(Permission.COMPLAINT_CREATE)), c: AppContainer = Depends(get_container), _: None = Depends(limited("expensive"))) -> dict:
...
def classify_preview(body: ClassifyPreviewBody, ctx: AuthContext = Depends(guard(Permission.COMPLAINT_CREATE)), c: AppContainer = Depends(get_container)) -> dict:
...
async def upload_evidence(file: UploadFile = File(...), ctx: AuthContext = Depends(guard(Permission.EVIDENCE_UPLOAD)), c: AppContainer = Depends(get_container), _: None = Depends(limited("upload"))) -> dict:
    data = await file.read(c.settings.max_upload_bytes + 1)
    return to_jsonable(c.complaints.upload_evidence(ctx, file.filename or "upload", data, file.content_type))
...
def evidence_analyze(evidence_id: str, ctx: AuthContext = Depends(guard(Permission.EVIDENCE_UPLOAD)), c: AppContainer = Depends(get_container)) -> dict:
    return to_jsonable(c.complaints.request_analysis(ctx, evidence_id))


@router.post("/drafts")
def save_draft(body: DraftBody, ctx: AuthContext = Depends(guard(Permission.COMPLAINT_CREATE)), c: AppContainer = Depends(get_container)) -> dict:
    return to_jsonable(c.drafts.save(ctx, body.kind, body.payload, body.client_request_id))


@router.get("/drafts")
def list_drafts(ctx: AuthContext = Depends(guard(Permission.COMPLAINT_CREATE)), c: AppContainer = Depends(get_container)) -> dict:
    return {"items": to_jsonable(c.drafts.list(ctx))}


@router.post("/drafts/{draft_id}/pending")
def mark_pending(draft_id: str, ctx: AuthContext = Depends(guard(Permission.COMPLAINT_CREATE)), c: AppContainer = Depends(get_container)) -> dict:
    return to_jsonable(c.drafts.mark_pending(ctx, draft_id))


@router.post("/drafts/{draft_id}/sync")
def sync_draft(draft_id: str, ctx: AuthContext = Depends(guard(Permission.COMPLAINT_CREATE)), c: AppContainer = Depends(get_container)) -> dict:
    return to_jsonable(c.drafts.sync(ctx, draft_id))
...
def attach_evidence(complaint_id: str, body: AttachEvidenceBody, ctx: AuthContext = Depends(guard(Permission.EVIDENCE_UPLOAD)), c: AppContainer = Depends(get_container)) -> dict:
    return to_jsonable(c.complaints.add_evidence(ctx, complaint_id, body.evidence_id))


@router.get("/{complaint_id}")
def detail(complaint_id: str, ctx: AuthContext = CITIZEN, c: AppContainer = Depends(get_container)) -> dict:
...
def government_states(complaint_id: str, ctx: AuthContext = CITIZEN, c: AppContainer = Depends(get_container)) -> dict:
...
def request_government_submission(complaint_id: str, body: GovSubmitBody, ctx: AuthContext = Depends(guard(Permission.COMPLAINT_READ_OWN)), c: AppContainer = Depends(get_container)) -> dict:
    return to_jsonable(c.government.request(ctx, complaint_id, body.platform))
