...
@router.post("/analyze")
def analyze(body: LegalBody, ctx: AuthContext = LEGAL, c: AppContainer = Depends(get_container), _: None = Depends(limited("expensive"))) -> dict:
    return _view(c.legal.analyze(ctx, body.problem, disposal=body.disposal, year=body.year))


@router.post("/analyze-document")
async def analyze_document(file: UploadFile = File(...), ctx: AuthContext = LEGAL, c: AppContainer = Depends(get_container), _: None = Depends(limited("upload"))) -> dict:
...
@router.get("/analyses")
def analyses(ctx: AuthContext = LEGAL, c: AppContainer = Depends(get_container)) -> dict:
...
def analysis(analysis_id: str, ctx: AuthContext = LEGAL, c: AppContainer = Depends(get_container)) -> dict:
    return _view(c.legal.get_mine(ctx, analysis_id))


@router.post("/precedents/search")
def search_precedents(body: LegalBody, ctx: AuthContext = LEGAL, c: AppContainer = Depends(get_container)) -> dict:
...
def precedent(cnr: str, ctx: AuthContext = LEGAL, c: AppContainer = Depends(get_container)) -> dict:
