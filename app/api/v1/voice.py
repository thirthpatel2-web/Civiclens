...
async def transcribe(audio: UploadFile = File(...), language: str = Form("auto"), ctx: AuthContext = Depends(guard(Permission.COMPLAINT_CREATE)), c: AppContainer = Depends(get_container), _: None = Depends(limited("upload"))) -> dict:
    data = await audio.read(10 * 1024 * 1024 + 1)
    return to_jsonable(c.voice.transcribe(ctx, data, audio.content_type, language))


@router.post("/translate")
def translate(body: TranslateBody, ctx: AuthContext = Depends(guard(Permission.ASSISTANT_USE)), c: AppContainer = Depends(get_container), _: None = Depends(limited("expensive"))) -> dict:
...
def languages(ctx: AuthContext = Depends(guard(Permission.COMPLAINT_CREATE)), c: AppContainer = Depends(get_container)) -> dict:
...
@router.get("/status")
def status(ctx: AuthContext = Depends(guard(Permission.COMPLAINT_CREATE)), c: AppContainer = Depends(get_container)) -> dict:
