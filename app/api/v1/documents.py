...
def _ingestor(c: AppContainer, uow):
...
async def upload(file: UploadFile = File(...), visibility: str = Form("private"), ctx: AuthContext = UP, c: AppContainer = Depends(get_container), _: None = Depends(limited("upload"))) -> dict:
...
@router.get("")
def list_mine(ctx: AuthContext = UP, c: AppContainer = Depends(get_container)) -> dict:
    return {"items": to_jsonable(run_in_uow(c, lambda uow: uow.documents.list_for_owner(ctx.user_id)))}


def _own(uow, ctx: AuthContext, doc_id: str):
...
@router.get("/{doc_id}")
def status(doc_id: str, ctx: AuthContext = UP, c: AppContainer = Depends(get_container)) -> dict:
    return to_jsonable(run_in_uow(c, lambda uow: _own(uow, ctx, doc_id)))


@router.post("/{doc_id}/retry", status_code=202)
def retry(doc_id: str, ctx: AuthContext = UP, c: AppContainer = Depends(get_container)) -> dict:
    def op(uow):
...
def link(doc_id: str, body: LinkBody, ctx: AuthContext = UP, c: AppContainer = Depends(get_container)) -> dict:
...
@router.post("/search")
def search(body: SearchBody, ctx: AuthContext = Depends(guard(Permission.ASSISTANT_USE)), c: AppContainer = Depends(get_container), _: None = Depends(limited("expensive"))) -> dict:
