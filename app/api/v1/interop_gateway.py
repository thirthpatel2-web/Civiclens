...
exceptions surface) - this router is the ADMIN_INTEGRATIONS-gated console over
InteropGatewayService: trigger a document exchange, act on the consent it requires, resolve
ambiguous identity matches, and inspect the connector registry / transaction / timeline history
that resulted. See app/services/interop_gateway_service.py for what each endpoint actually does.
"""
...
ADMIN = Depends(guard(Permission.ADMIN_INTEGRATIONS))


@router.post("/document-exchange")
def request_document_exchange(body: RequestDocumentExchangeBody, ctx: AuthContext = ADMIN, c: AppContainer = Depends(get_container)) -> dict:
...
def get_timeline(application_no: str, ctx: AuthContext = ADMIN, c: AppContainer = Depends(get_container)) -> dict:
    return to_jsonable(c.interop_gateway.get_timeline(ctx, application_no=application_no))


@router.get("/transactions")
def list_transactions(limit: int = 50, ctx: AuthContext = ADMIN, c: AppContainer = Depends(get_container)) -> dict:
    return {"items": to_jsonable(c.interop_gateway.list_transactions(ctx, limit=limit))}


# ---- consent lifecycle
@router.get("/consents")
def list_consents(status: str | None = None, ctx: AuthContext = ADMIN, c: AppContainer = Depends(get_container)) -> dict:
    return {"items": to_jsonable(c.interop_gateway.list_consents(ctx, status=status))}


@router.post("/consents/{consent_id}/grant")
def grant_consent(consent_id: str, ctx: AuthContext = ADMIN, c: AppContainer = Depends(get_container)) -> dict:
    return to_jsonable(c.interop_gateway.grant_consent(ctx, consent_id=consent_id))


@router.post("/consents/{consent_id}/deny")
def deny_consent(consent_id: str, ctx: AuthContext = ADMIN, c: AppContainer = Depends(get_container)) -> dict:
    return to_jsonable(c.interop_gateway.deny_consent(ctx, consent_id=consent_id))


@router.post("/consents/{consent_id}/revoke")
def revoke_consent(consent_id: str, body: RevokeConsentBody, ctx: AuthContext = ADMIN, c: AppContainer = Depends(get_container)) -> dict:
...
def list_identity_candidates(status: str = "pending", ctx: AuthContext = ADMIN, c: AppContainer = Depends(get_container)) -> dict:
...
def resolve_identity_candidate(candidate_id: str, body: ResolveIdentityCandidateBody, ctx: AuthContext = ADMIN, c: AppContainer = Depends(get_container)) -> dict:
    return to_jsonable(c.interop_gateway.resolve_identity_candidate(ctx, candidate_id=candidate_id, approve=body.approve))


# ---- connector registry
@router.get("/connectors")
def list_connectors(ctx: AuthContext = ADMIN, c: AppContainer = Depends(get_container)) -> dict:
    return {"items": to_jsonable(c.interop_gateway.list_connectors(ctx))}


@router.post("/connectors/{connector_id}/health-check")
def connector_health(connector_id: str, ctx: AuthContext = ADMIN, c: AppContainer = Depends(get_container)) -> dict:
...
def set_connector_enabled(connector_id: str, body: SetConnectorEnabledBody, ctx: AuthContext = ADMIN, c: AppContainer = Depends(get_container)) -> dict:
