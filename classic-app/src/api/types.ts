...
export interface CitizenDashboard {
  has_data: boolean; total: number; open: number; resolved: number; escalated: number; backlog: number; unrouted: number;
  resolution_hours: { count: number; mean: number; median: number } | null;
  recent: Complaint[]; pending_feedback: string[]; unread_notifications: number;
  rti_countdowns: Array<{ reference: string | null; due_at: string | null; days_left: number | null; estimate: boolean }>;
}
